...
def is_relevant_strict(query: str, title: str, summary: str, content: str, url: str) -> bool:
    ...
    if len(q.split()) >= 2 and q in hay:  # frasa persis (nama 2 kata, dsb)
        return True
    ...
    try:
        if _has_tz_info(dt_str):
            ...
            if ts is not None and not pd.isna(ts):
                ts = ts.tz_convert(TZ_JKT); return ts.strftime("%Y-%m-%d %H:%M")
        else:
            ts = pd.to_datetime(dt_str, errors="coerce")
            if ts is not None and not pd.isna(ts):
                ts = TZ_JKT.localize(ts) if ts.tzinfo is None else ts.tz_convert(TZ_JKT)
...
def scrape_detik(query, max_articles=15):
        ...
        if res.status_code == 200:
            ...
            for it in items:
                try:
                    ...
                    data.append({"source":"Detik","title":title,"description":description,"content":(title+" "+description+" "+content).strip(),"url":link,"publishedAt":published_at})
    ...
    if len(data) < max_articles:
        feeds = [
            "https://news.detik.com/rss","https://finance.detik.com/rss","https://sport.detik.com/rss","https://hot.detik.com/rss",
        ...
        try:
            for fu in feeds:
                ...
                for e in feed.entries:
                    ...
                    data.append({"source":"Detik","title":title,"description":summary,"content":(title+" "+summary+" "+content).strip(),"url":link,"publishedAt":published_at})
        except Exception:
            pass
...
def scrape_cnn_fixed(query, max_results=12):
    urls = [
        ...
        "https://www.cnnindonesia.com/ekonomi/rss","https://www.cnnindonesia.com/olahraga/rss","https://www.cnnindonesia.com/gaya-hidup/rss",
    ]
                ...
                if res.status_code == 200:
                    ...
                    for article in articles:
                        ...
                        try:
                            ...
                            link = link_tag['href']
                            ttl_el = link_tag.find('span', class_='box--card__title'); title = ttl_el.get_text(strip=True) if ttl_el else ""
                            sum_el = article.find('span', class_='box--card__desc'); summary = sum_el.get_text(strip=True) if sum_el else ""
                            pub, content = fetch_time_and_content(sess, link)
                            if not pub: continue
                            if not is_relevant_strict(query, title, summary, content, link): continue
                            results.append({"source":get_source_from_url(link),"title":title,"description":summary,"content":(title+" "+summary+" "+content).strip(),"url":link,"publishedAt":pub})
                        except Exception:
                            continue
        except Exception:
            continue
        ...
        if res.status_code == 200:
            ...
            for it in items[:max_articles]:
                try:
                    ...
                    data.append({"source":"Kompas","title":title,"description":"","content":(title+" "+content).strip(),"url":url,"publishedAt":pub})
                except Exception:
                    continue
    except Exception:
        pass
...
def save_interaction_to_github(user_id, query, all_articles, clicked_urls):
    ...
    repo.update_file(st.secrets["file_path"], f"Update history for {query}", updated_content, contents.sha)
...
def get_trending_topics(user_id, df, days=3, half_life_hours=12):
    ...
    return list(sc.items())

# ================== LINK LOGGER VIA QUERY PARAM ==================
def _enc(s):
...
def get_query_params():
    try:
        ...
        return st.experimental_get_query_params()  # Lama
...
# ================== APP ==================
def main():
    # ---- intercept query-param untuk logging klik & buka tab ----
    ...
    if "log" in params:
        ...
        url = _dec(raw_log); qx = _dec(raw_q)
    ...
    df_train = build_training_data(USER_ID := "user_01")
    clf = None
        ...
        for date, queries in grouped_queries.items():
            ...
            for q in unique_queries:
                with st.expander(f"- {q}", expanded=False):
                    ...
                    # Rekomendasi (preset default, TANPA tombol baca selengkapnya & TANPA ringkasan hitung sumber)
        ...
        if df_news.empty:
            ...
            if results.empty:
                ...
                for _, row in results.iterrows():
                    ...
                    skor_val = row.get('final_score', row.get('similarity', row.get('sbert_score', 0.0)))
                    st.write(f"Skor: `{float(skor_val):.2f}`")
                    # Di bagian ini logging klik tetap aktif
    ...
    if st.button("Cari Berita"):
        if search_query:
            ...
            with st.spinner('Mengambil berita dan merekomendasikan...'):
                ...
                results = recommend(
                    st.session_state.current_search_results, search_query, clf,
                    n_per_source=3,
                    min_score=DEFAULT_MIN_SCORE,
                    use_lr_boost=USE_LR_BOOST, alpha=ALPHA,
                    per_source_group=PER_SOURCE_GROUP
